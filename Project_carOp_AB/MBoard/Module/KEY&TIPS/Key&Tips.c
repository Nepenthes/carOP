...
void keyInit(void){	
	...
	RCC_APB2PeriphClockCmd(RCC_APB2Periph_GPIOB | RCC_APB2Periph_GPIOC,ENABLE);		//时钟分配
...
static uint16_t keyScan(void){
	...
	if(!K3)return KEY_VALUE_3;		//键值1
	if(!K4)return KEY_VALUE_4;		//键值2
	if(!K5)return KEY_VALUE_5;		//键值1
	if(!K6)return KEY_VALUE_6;		//键值2
	return KEY_NULL;					//无按键
}
