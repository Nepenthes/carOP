...
void OPR_12x345(void){

	const uint8_t FLEN = 5;			//查找表表长，即使用了几个灯条
	const uint8_t FORM_LEN = 3;		//变化个数
	const uint8_t FORM[FORM_LEN] = {2,3,4};	//变化条目索引
	uint8_t loop,loopa;
	uint8_t FrameR[FLEN] = {0};	//查找表
...
void OPG_9876(void){
	...
	const uint8_t FLEN = 9;			//查找表表长，即使用了几个灯条
	const uint8_t FORM_LEN = 4;		//变化个数
	const uint8_t FORM[FORM_LEN] = {8,7,6,5};	//变化条目索引
	uint8_t loop,loopa;
...
void OPG_9876U12x345(void){

	const uint8_t FLEN = 9;			//查找表表长，即使用了几个灯条
	const uint8_t FORM_LEN = 3;		//变化个数
	const uint8_t FORM1[FORM_LEN] = {7,6,5};	//变化条目索引		
	const uint8_t FORM2[FORM_LEN] = {2,3,4};	//变化条目索引
	uint8_t loop,loopa;
	uint8_t FrameG[FLEN] = {0};	//查找表
	...
	for(loop = 0;loop < FORM_LEN;loop++){
		...
		for(loopa = 0;loopa < 8;loopa++){
			...
			FrameG[FORM1[loop]] |= (0x80 >> loopa);
			FrameG[FORM2[loop]] |= (0x01 << loopa);
			frame_Send((uint8_t *)FrameG,FLEN,M2);
		}
	}	
...
void Task_KEYMT(void const *argument){
	...
	if(OPKEY != OP_KEYVAL){
		...
		switch(OPKEY){
			...
			case STATUS_SPNML:	tid_carOpG	=	osThreadCreate(osThread(carOperationG_thread), NULL);
								tid_carOpR	=	osThreadCreate(osThread(carOperationR_thread), NULL);
...
void carOperationG_thread(const void *argument){
	...
	for(;;){
		...
		switch(OP_KEYVAL){
			...
			case STATUS_START:	OPG_9876();
								break;
			
			case STATUS_SPLOW:	OPG_9876();
								break;
			
			case STATUS_SPNML:	OPG_9876();
								break;
			
			case STATUS_SPFUL:	OPG_9876();
								break;
			
			case STATUS_SPDOW:	OPG_9876U12x345();
								break;
			
			case STATUS_STOP :	OPG_9876();
								break;
...
void carOperationR_thread(const void *argument){
	...
	for(;;){
		...
		switch(OP_KEYVAL){
			...
			case STATUS_START:	OPR_543x12();
								break;
			
			case STATUS_SPLOW:	OPR_543x12();
								break;
			
			case STATUS_SPNML:	OPR_543x12();
								break;
			
			case STATUS_SPFUL:	OPR_543x12();
								break;
